...
const config = JSON.parse(
  fs.readFileSync(path.join(__dirname, "line.config.json"), {
    encoding: "utf-8"
  })
);
        ...
        if (stats.isFile() && !isIgnore) {
          ...
          rl.on("close", () => {
            ...
            if (!ignoreWarning && lines > warnLine) {
              console.warn(warning_c(`${target_file_path}行数${lines}`));
            }
          });
        }
      });
